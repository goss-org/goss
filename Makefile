...
gen-ci:
	$(info INFO: Starting build $@)
	if [ -z $${CI+x} ]; then echo 'not CI, skipping "make gen"'; else $(MAKE) gen; fi

gen:
	$(info INFO: Starting build $@)
	go generate -tags genny $(pkgs)
	[ $$(git status -s | grep -v "go.mod" | grep  -v "go.sum" | wc -l) -eq 0 ] || (git status -s; echo 'autogenerated files have updates'; exit 1)
