...
out=$(docker_exec "/goss/$os/goss-linux-$arch" --vars "/goss/vars.yaml" -g "/goss/$os/goss.yaml" validate -r 10s -s 100ms)
echo "$out"

if [[ $os == "arch" ]]; then
  egrep -q 'Count: 80, Failed: 0, Skipped: 3' <<<"$out"
else
  egrep -q 'Count: 96, Failed: 0, Skipped: 5' <<<"$out"
fi
