...
type BlockDeviceMap map[string]*BlockDevice

func (r BlockDeviceMap) AppendSysResource(sr string, sys *system.System, config util.Config) (*BlockDevice, error) {
	sysres := sys.NewBlockDevice(sr, sys, config)
	res, err := NewBlockDevice(sysres, config)
	if err != nil {
		return nil, err
	}
	if old_res, ok := r[res.ID()]; ok {
...
func (r BlockDeviceMap) AppendSysResourceIfExists(sr string, sys *system.System) (*BlockDevice, system.BlockDevice, bool) {
	sysres := sys.NewBlockDevice(sr, sys, util.Config{})
	// FIXME: Do we want to be silent about errors?
	res, _ := NewBlockDevice(sysres, util.Config{})
	if e, _ := sysres.Exists(); e != true {
		return res, sysres, false
	}
	if old_res, ok := r[res.ID()]; ok {
...
func (ret *BlockDeviceMap) UnmarshalJSON(data []byte) error {
	// Curried json.Unmarshal
	unmarshal := func(i interface{}) error {
		if err := json.Unmarshal(data, i); err != nil {
			return err
	...
	var tmp map[string]*BlockDevice
	if err := unmarshal(&tmp); err != nil {
		return err
	}
...
func (ret *BlockDeviceMap) UnmarshalYAML(unmarshal func(v interface{}) error) error {
	// Validate configuration
	zero := BlockDevice{}
	whitelist, err := util.WhitelistAttrs(zero, util.YAML)
	if err != nil {
