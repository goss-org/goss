...
func (r EnvVarMap) AppendSysResource(sr string, sys *system.System, config util.Config) (*EnvVar, error) {
	sysres := sys.NewEnvVar(sr, sys, config)
	res, err := NewEnvVar(sysres, config)
	if err != nil {
		return nil, err
	}
	if old_res, ok := r[res.ID()]; ok {
...
func (r EnvVarMap) AppendSysResourceIfExists(sr string, sys *system.System) (*EnvVar, system.EnvVar, bool) {
	sysres := sys.NewEnvVar(sr, sys, util.Config{})
	// FIXME: Do we want to be silent about errors?
	res, _ := NewEnvVar(sysres, util.Config{})
	if e, _ := sysres.Exists(); e != true {
		return res, sysres, false
	}
	if old_res, ok := r[res.ID()]; ok {
...
func (ret *EnvVarMap) UnmarshalJSON(data []byte) error {
	// Curried json.Unmarshal
	unmarshal := func(i interface{}) error {
		if err := json.Unmarshal(data, i); err != nil {
			return err
...
func (ret *EnvVarMap) UnmarshalYAML(unmarshal func(v interface{}) error) error {
	// Validate configuration
	zero := EnvVar{}
	whitelist, err := util.WhitelistAttrs(zero, util.YAML)
	if err != nil {
